...
const buttonVariants = cva(
  cn(
    ...
    Platform.select({
      web: "focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive outline-none transition-colors focus-visible:ring-[3px] disabled:pointer-events-none [&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
    }),
  ),
  {
    variants: {
      ...
      size: {
        ...
        sm: cn(
          "min-h-12 min-w-12 gap-1.5 rounded-md px-3 py-3",
          Platform.select({ web: "has-[>svg]:px-2.5" }),
        ),
        lg: cn(
          "min-h-14 min-w-14 rounded-md px-6 py-4",
...
const buttonTextVariants = cva(
  cn(
    "text-foreground text-base font-semibold text-center shrink",
    ...
    <TextClassContext.Provider value={buttonTextVariants({ variant, size })}>
      <Pressable
        className={cn(
          ...
          !reduced && "transition-transform duration-100 active:scale-[0.98]",
          className,
        )}
        role={props.accessibilityRole === "radio" ? "radio" : "button"}
