...
export function AppThemeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function loadPreference() {
      ...
      try {
        ...
        if (active) setStorageError('Your saved appearance could not be loaded.');
      }
      if (!active) return;
  ...
  function setPreference(next: ThemePreference) {
    ...
    void AsyncStorage.setItem(STORAGE_KEY, next).catch(() => {
      setStorageError('Appearance changed, but could not be saved for next time.');
    });
  }
  ...
  return (
    <ThemePreferenceContext.Provider value={{ preference, setPreference, storageError }}>
