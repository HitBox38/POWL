...
async function main() {
  const source = await fs.readFile(path.join(imageDir, 'powl-mark.svg'), 'utf8');
  const artwork = source.replace(/<svg[^>]*>|<\/svg>|<title>[\s\S]*?<\/title>|<desc>[\s\S]*?<\/desc>/g, '').trim();
  const svg = (body) => `<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512" fill="none">\n${body}\n</svg>\n`;
  ...
  const adaptive = svg(`<g transform="translate(256 256) scale(0.85) translate(-256 -266)">${artwork}</g>`);
  const splash = svg(`<g transform="translate(0 -10)">${artwork}</g>`);
  const monochrome = adaptive.replace(/#38BDF8|#F0F9FF/g, '#FFFFFF');
  ...
  const render = (input, name, size, opaque = false) => {
    ...
    return (opaque ? output.removeAlpha() : output).png().toFile(path.join(imageDir, name));
  };
  await Promise.all([
  ...
  if (process.argv.includes('--android')) {
    ...
    const roundIcon = icon.replace('<rect width="512" height="512"', '<rect width="512" height="512" rx="256"');
    const exports = {
    ...
    for (const folder of await fs.readdir(resDir)) {
      if (!/^(mipmap|drawable)-(mdpi|hdpi|xhdpi|xxhdpi|xxxhdpi)$/.test(folder)) continue;
      for (const file of await fs.readdir(path.join(resDir, folder))) {
        ...
        const rendered = sharp(Buffer.from(exports[file])).resize(width, height);
        const buffer = await (file.endsWith('.webp') ? rendered.webp({ lossless: true }) : rendered.png()).toBuffer();
